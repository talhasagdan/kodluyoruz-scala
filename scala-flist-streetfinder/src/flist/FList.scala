...
sealed trait FList[+A] {
  ...
  override def toString :String = this match {
    case FNil => ""
    case FNonNil(head,tail) =>  head + " " + tail
  }
   ...
   */
  def append[AA >: A](element: AA): FList[AA] = this match {
   ...
   */
  def prepend[AA >: A](element: AA): FList[AA] = this match {
      case FNil => FNonNil(element,FNil)
      case FNonNil(head,tail) =>  FNonNil(element, tail.prepend(head))
   ...
   */
  def concat[AA >: A](that: FList[AA]): FList[AA] = this match {
    case FNil => that
   ...
   */
  def map[B](mapper: A => B): FList[B] = this match {
      case FNil                => FNil
   ...
   */
  def flatMap[B](flatMapper: A => FList[B]): FList[B] = this match {
    case FNil => FNil
   ...
   */
  def get(index: Int): Option[A] = this match {
    case FNil => None
  ...
  def fold[B](initial: B)(step: (B, A) => B): B = this match {
    case FNil => initial
   ...
   */
  def filter(predicate: A => Boolean): FList[A] = this match {
    case FNil=>FNil
    case FNonNil(head,tail) =>
      if (predicate(head)) {
        FNonNil(head,tail.filter(predicate))
      }
    else{
